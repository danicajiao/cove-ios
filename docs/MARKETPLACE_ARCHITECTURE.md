...
These are defined in migrations `000005` and `000006`. The v1 seed (`000004`) inserts all ~140 nodes; the backfill in `000005` sets `is_leaf = false` on all non-leaf nodes so the column is consistent from the start.
...
- **Version 4.4** (May 2026) — **Category leaf enforcement.** Added `is_leaf boolean` to `catalog.categories`; four triggers enforce that items only reference leaf nodes and keep `is_leaf` accurate on insert/delete. v1 seed (~140 nodes across 9 top-level categories) landed in migrations 004–006. Added [Category Taxonomy](CATEGORY_TAXONOMY.md) reference doc.
